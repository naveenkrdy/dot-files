...
name: visual-plan
description: >-
  Only use when the user explicitly runs /visual-plan or asks for it by name
  — do not auto-trigger based on task shape. Turns ordinary text plans into
  rich interactive visual plans with diagrams, file maps, annotated code,
  open questions, and UI/prototype review.
metadata:
  visibility: exported
---
...
Only when the user explicitly runs `/visual-plan` or asks for a visual plan
by name. Do not switch to this mode on your own judgment of what the task
would benefit from, even for UI-heavy or ambiguous work — use a normal text
plan and let the user opt in.
